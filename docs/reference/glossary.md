...
```{glossary}
compute
: The processing power a computer uses to perform tasks. This usually refers to either {term}`CPU` or {term}`RAM`.

CPU
: Central Processing Unit. Executes tasks and processes data.

Byte-hour
: byte-hour. A base unit of compute for memory usage. E.g. a server with 4GiB RAM running for 24 hours consumes 4 $\times$ 24 = 96 GiB-hours of memory usage.
